...
from diff_abm_traffic.networks import build_network_chicago


def main() -> None:
    config = load_config_from_env()
    opts = CalibrationOptions(
        network_builder=lambda script_dir: build_network_chicago(script_dir=script_dir),
        city_label="Chicago network",
        adjust_obs_schedule=False, 
    )
